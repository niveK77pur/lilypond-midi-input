type Alteration<'a> = (u8, &'a str);

#[derive(Debug)]
pub struct LilyParameters<'a> {
    key: LilyKeySignature,
    /// custom alterations within an octave (0-11)
    alterations: Vec<Alteration<'a>>,
    /// custom alterations over all notes
    global_alterations: Vec<Alteration<'a>>,
}

impl<'a> LilyParameters<'a> {
    pub fn new(
        key: LilyKeySignature,
        alterations: Vec<Alteration<'a>>,
        global_alterations: Vec<Alteration<'a>>,
    ) -> Self {
        LilyParameters {
            key,
            alterations,
            global_alterations,
        }
    }
    pub fn key(&self) -> &LilyKeySignature {
        &self.key
    }
    pub fn alterations(&self) -> &Vec<Alteration<'a>> {
        &self.alterations
    }
    pub fn global_alterations(&self) -> &Vec<Alteration<'a>> {
        &self.global_alterations
    }
}

#[derive(Debug)]
/// List of possible musical key signatures
pub enum LilyKeySignature {
    CFlatMajor,  // 7 flats
    GFlatMajor,  // 6 flats
    DFlatMajor,  // 5 flats
    AFlatMajor,  // 4 flats
    EFlatMajor,  // 3 flats
    BFlatMajor,  // 2 flats
    FMajor,      // 1 flat
    CMajor,      // 0 flats/sharps
    GMajor,      // 1 sharp
    DMajor,      // 2 sharps
    AMajor,      // 3 sharps
    EMajor,      // 4 sharps
    BMajor,      // 5 sharps
    FSharpMajor, // 6 sharps
    CSharpMajor, // 7 sharps
    AFlatMinor,  // 7 flats
    EFlatMinor,  // 6 flats
    BFlatMinor,  // 5 flats
    FMinor,      // 4 flats
    CMinor,      // 3 flats
    GMinor,      // 2 flats
    DMinor,      // 1 flat
    AMinor,      // 0 flats/sharps
    EMinor,      // 1 sharp
    BMinor,      // 2 sharps
    FSharpMinor, // 3 sharps
    CSharpMinor, // 4 sharps
    GSharpMinor, // 5 sharps
    DSharpMinor, // 6 sharps
    ASharpMinor, // 7 sharps
}

#[derive(Debug)]
pub struct LilyNote {
    /// the LilyPond note string
    letter: &'static str,
    /// octave indication is very small, we do not need a large integer
    octave: i8,
}

impl LilyNote {
    pub fn new(value: u8, parameters: LilyParameters) -> Self {
        LilyNote {
            letter: Self::note_name(value % 12, &parameters).expect("Note within octave"),
            octave: (value as i16 / 12) as i8 - 4,
        }
    }

    fn note_name(note: u8, parameters: &LilyParameters) -> Result<&'static str, LilypondNoteError> {
        let LilyParameters { key, .. } = parameters;
        match note {
            0 => match key {
                LilyKeySignature::CSharpMajor => Ok("bis"),
                LilyKeySignature::CSharpMinor => Ok("bis"),
                _ => Ok("c"),
            },
            1 => match key {
                LilyKeySignature::AFlatMajor => Ok("des"),
                LilyKeySignature::DMajor => Ok("cis"),
                LilyKeySignature::DMinor => Ok("cis"),
                _ => Ok("cis"),
            },
            2 => match key {
                LilyKeySignature::EFlatMinor => Ok("d"),
                LilyKeySignature::DSharpMinor => Ok("cisis"),
                _ => Ok("d"),
            },
            3 => match key {
                LilyKeySignature::BFlatMajor => Ok("ees"),
                LilyKeySignature::EMajor => Ok("dis"),
                LilyKeySignature::EMinor => Ok("dis"),
                _ => Ok("dis"),
            },
            4 => match key {
                LilyKeySignature::CFlatMajor => Ok("fes"),
                LilyKeySignature::FMinor => Ok("e"),
                _ => Ok("e"),
            },
            5 => match key {
                LilyKeySignature::FSharpMajor => Ok("eis"),
                LilyKeySignature::FSharpMinor => Ok("eis"),
                _ => Ok("f"),
            },
            6 => match key {
                LilyKeySignature::DFlatMajor => Ok("ges"),
                LilyKeySignature::GMajor => Ok("fis"),
                LilyKeySignature::GMinor => Ok("fis"),
                _ => Ok("fis"),
            },
            7 => match key {
                LilyKeySignature::AFlatMinor => Ok("g"),
                LilyKeySignature::GSharpMinor => Ok("fisis"),
                _ => Ok("g"),
            },
            8 => match key {
                LilyKeySignature::EFlatMajor => Ok("aes"),
                LilyKeySignature::AMajor => Ok("gis"),
                LilyKeySignature::AMinor => Ok("gis"),
                _ => Ok("gis"),
            },
            9 => match key {
                LilyKeySignature::BFlatMinor => Ok("a"),
                LilyKeySignature::ASharpMinor => Ok("gisis"),
                _ => Ok("a"),
            },
            10 => match key {
                LilyKeySignature::FMajor => Ok("bes"),
                LilyKeySignature::BMajor => Ok("ais"),
                LilyKeySignature::BMinor => Ok("ais"),
                _ => Ok("ais"),
            },
            11 => match key {
                LilyKeySignature::GFlatMajor => Ok("ces"),
                LilyKeySignature::CMinor => Ok("b"),
                _ => Ok("b"),
            },
            _ => Err(LilypondNoteError::OutsideOctave),
        }
    }
}

#[derive(Debug)]
pub enum LilypondNoteError {
    /// Some functions require the note to be within an octave (integer between 0 to 11)
    OutsideOctave,
}
